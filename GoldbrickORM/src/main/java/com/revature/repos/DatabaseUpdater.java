...
public class DatabaseUpdater {
	...
	public static void updateProduct(Metamodel<Product> mm, Product product, Connection conn) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException {
		IdField pk = mm.getPrimaryKey();
		Map<String, Method> getters = mm.getGetters();
		...
		for (String columnName : getters.keySet()) {
			if (!columnName.equals(pk.getColumnName()))
				sql += columnName + " = " + getters.get(columnName).invoke(product, null) + ", ";
		}
	...
	public static void main(String[] args) {
		...
		try {
			...
			Product prod = new Product(1, 1, "test2", "test2", 0, 0, false);
